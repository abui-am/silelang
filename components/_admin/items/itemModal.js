...
const ItemModal = (props) => {
  ...
  const getImages = async () => {
    const images = await getApi().get(`items/${props.item.id_barang}/images`);
    ...
    <Box className={classes.root}>
      <Box style={{ position: "absolute", top: 25, right: 25 }}>
        <Fab
          ...
          aria-label="edit"
          onClick={() => {
            router.push(`/_admin/items/${item.id_barang}/edit`);
          }}
        >
          <EditIcon />
        </Fab>
      ...
      <Box className={classes.titleBox} marginBottom={3}>
        <Typography className={classes.title}>Detail Barang</Typography>
      </Box>
        ...
        <Box className={classes.sectionList}>
          ...
          <Box marginBottom={2}>
            <Typography
              ...
              style={{ color: "rgba(18, 18, 18, 0.8)" }}
            >
              {dayjs(item.tgl).format("dddd, DD MMM YYYY")}
            </Typography>
          </Box>
        </Box>
        <Box className={classes.sectionList}>
        ...
        <Box className={classes.sectionList}>
          ...
          <Box marginBottom={2}>
            <Typography
              ...
              style={{ color: "rgba(18, 18, 18, 0.8)" }}
            >
              {item.terjual === 0 ? "Belum terjual" : "Sudah terjual"}
            </Typography>
          </Box>
        </Box>
      </Box>
